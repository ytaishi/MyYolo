...
def binarize(image):
    th = cv2.adaptiveThreshold(
        image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, 20
    )
    return th
...
if __name__ == "__main__":
    ...
    while True:
        ...
        blur_image = preprocess_mask(gray_frame)  # 前処理 (ぼかし)
        binary_image = binarize(blur_image)  # 適応的な閾値処理
        # mask = detect_white(blur_image)  # 白色を検出
        detected_lines = fld.detect(binary_image)
        ...
        lined_image = draw_lines(frame, long_lines)  # 線分を描画

        cv2.imshow("Original Image", frame)
        cv2.imshow("White Lines Detected", lined_image)
        cv2.imshow("Binary Image", binary_image)
        cv2.imshow("Blur Lines Detected", blur_image)
